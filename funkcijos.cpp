...
void rusiavimas (vector <studentas> &temp, vector <studentas> &vargsas, vector <studentas> &kietas, string failodydis, double &rusiavim) {
    ...
    sort(vargsas.begin(), vargsas.end(), palyginimas);
    sort(kietas.begin(), kietas.end(), palyginimas);


    end = std::chrono::high_resolution_clock::now();
    diff = end - start;
    std::cout << failodydis << " elementu failo rusiavimas vargsu ir kietu uztruko: "<< fixed << setprecision(2)<< diff.count() << " s\n";
