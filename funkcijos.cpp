...
void rusiavimas (vector <studentas> &temp, vector <studentas> &vargsas, vector <studentas> &kietas, string failodydis, double &rusiavim) {
    ...
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff = end - start;
    std::cout << failodydis << " elementu failo padalijimas i vargsus ir kietus: "<< diff.count() << " s\n";
    ...
    diff = end - start;
    std::cout << failodydis << " elementu failo rusiavimas vargsu ir kietu uztruko: "<< diff.count() << " s\n";
