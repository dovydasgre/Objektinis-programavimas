...
void skaitymas (studentas &temp, vector <studentas> &mas) {
    ...
    try {
        ...
        cout << "KLAIDA! Duomenu failo pavadinimu "<< klaida << " nera." << endl;
        cout << "Bandykite paleisti programa is naujo ir pasitikrinkite ar gerai ivedete pavadinima arba ar failas tikrai egzistuoja." << endl;
        exit (0);
  }
    std::ifstream open_f(df);
