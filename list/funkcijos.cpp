...
void rusiavimas (list <studentas> &temp, list <studentas> &vargsas, list <studentas> &kietas, string failodydis, double &rusiavim) {
    ...
    for (auto& i : temp){
        i.bendr=vidurkis(i, 1);
    }

    auto start = std::chrono::high_resolution_clock::now();

    temp.sort(palyginimas);
    ...
    if (i.bendr < 5){
        vargsas.push_back(i);
        } else kietas.push_back(i);
    }
    end = std::chrono::high_resolution_clock::now();
    diff = end - start;
    std::cout << failodydis << " elementu failo padalijimas i vargsus ir kietus: "<< fixed << setprecision(2) << diff.count() << " s\n";
...
void isvedimas2 (list <studentas> &temp, string failodydis, string tipas, double &isved) {
    ...
    for(auto& i : temp) {
        sprintf(eil, "%-15s%-15s", i.vardas.c_str(), i.pavarde.c_str());
        laikeilute=eil;
        laikeilute+="   "+to_string(i.bendr);
        out << laikeilute << endl;
